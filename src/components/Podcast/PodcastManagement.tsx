...
import { Plus, Calendar, Users, MessageSquare, FileText, Sparkles, Send, Check, X, Eye, EyeOff, Clock, Play, Edit as EditIcon, Trash2, CheckSquare } from 'lucide-react';
...
export default function PodcastManagement() {
  ...
  const [mainTab, setMainTab] = useState<'episodes' | 'approvals'>('episodes');
  ...
  const deleteEpisode = async (episodeId: string, episodeTitle: string) => {
    const confirmed = window.confirm(
      `Weet je zeker dat je episode "${episodeTitle}" wilt verwijderen?\n\nDit verwijdert ook alle vragen, notes en andere data van deze episode.`
    );
    ...
    if (error) {
      alert('Fout bij verwijderen episode: ' + error.message);
      return;
    }
    ...
    loadEpisodes();
    alert('Episode succesvol verwijderd');
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {mainTab === 'approvals' && <ProposalApproval />}
